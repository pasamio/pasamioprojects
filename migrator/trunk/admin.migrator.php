...
function makeDumps($option){
    global $tables, $mosConfig_absolute_path, $mosConfig_db, $mosConfig_dbprefix, $database,
           $autoinc, $drop, $exists, $sql_compat, $gzip, $fullinserts, $run_time, $delaypersession,
           $core_tables, $exclude_tables;
    ...
    switch ($dump_stage){
        ...
        case 2:
           if ((!isset($_SESSION['tables'])) && (!isset($_SESSION['table']))){
              ...
              $i = 0;
              foreach ($table_lists as $table){
                      if ((!in_array($table->Name, $core_tables)) && (!in_array($table->Name, $exclude_tables))) $_SESSION['tables'][] = $table->Name;
              }
           }
