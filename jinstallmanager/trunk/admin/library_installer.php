...
class JInstallerLibrary extends JInstaller {
	...
	function install()
	{
		...
		// Grab the files
		$this->files =& $this->manifest->getElementByPath('files');
		
		// Set the installation target paths
		$this->parent->setPath('extension_path', JPath::clean(JPATH_SITE.DS."libraries"));
		 ...
		 * ---------------------------------------------------------------------------------------------
		 */
		if( ! is_a($this->files, 'JSimpleXMLElement') ) {
			JError::raiseWarning(1, 'Library Install: '.JText::_('The XML file did not contain a files element'));
			return false;
		}
