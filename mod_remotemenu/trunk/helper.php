 ...
 * @since		1.5
 */
class modRemoteMenuHelper
{
	// modRemoteMenuHelper::getMenu implementation
	function getMenu() {
		static $menu;
		if(!isset( $menu )) {
			$name = 'Site';
			$options = Array('remote'=>1); // dummy
			//Load the router object
			$info =& JApplicationHelper::getClientInfo('site', true);
	...
	{
		$menu = new JRemoteMenuTree($params);
		$items = &modRemoteMenuHelper::getMenu();
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_remotemenu');
			$string = $cache->call(array('modRemoteMenuHelper', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
		...
		$menu	= &modRemoteMenuHelper::getMenu();
	...
	function render(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			default :
				...
				$xml = modRemoteMenuHelper::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class JRemoteMenuTree extends JTree
{
	/**
    ...
    $tmp = clone($newItem);
		...
		if ($tmp->url != null)
		{
			...
			if ($tmp->home == 1) {
				$tmp->url = $params->get('base',JURI::base());
			} elseif (strcasecmp(substr($tmp->url, 0, 4), 'http') && (strpos($tmp->link, 'index.php?') !== false)) {
				$tmp->url = JRoute::_($tmp->url, true, $iSecure);
			} elseif (!strcasecmp(substr($tmp->url, 0, 4), 'http') && substr($tmp->url, 0,1) != '/') {
				$tmp->url = $params->get('base',JURI::base()).'/'.str_replace('&', '&amp;', $tmp->url);
			} else {
