...
if(!defined('PACKAGE_MANIFEST_PATH')) define('PACKAGE_MANIFEST_PATH',JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_jpackageman' . DS .'manifests');
...
class JInstallerPackage extends JObject
{
	...
	function install()
	{
		...
		$manifest['dest'] = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_jpackageman'.DS.'manifests'.DS.basename($this->parent->getPath('manifest'));
...
//							JText::_('There was an error removing an extension!') . ' ' .
			...
			JError::raiseWarning(100, JText::_('Package').' '.JText::_('Uninstall').': '. 
				JText_('Manifest File invalid or not found'));
			return false;
		}
