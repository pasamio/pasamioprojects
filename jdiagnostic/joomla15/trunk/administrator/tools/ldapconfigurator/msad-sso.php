...
enter in the server you wish Joomla! to use. This requires your DNS to be configure properly.</p>
	...
	<input type="hidden" name="screen" value="msad-sso">
	<?php
}
...
function step2() {
	...
	to allow searching from an anonymous bind. This is used in the extension to bind against the directory
	when it is required, typically for situations such as SSO where we don't have any other valid credentials.
	This should be a service account of some variety that can log into your Active Directory via LDAP.</p>
	<p>Enter the login of the user you wish to connect with using their user name. This should come together
	with your site name to make the user principal name 	(e.g. username@your.site.name.com).</p>
	<table>
	<tr><td><b>Connect Username:</b></td><td><input type="text" name="username" value="<?php echo $username ?>" />@<?php echo $sitename ?></td></tr>
...
function step3() {
	...
	<tr><td><b>Default Settings:</b></td><td><ul><li>Auth Method: Search and Bind</li><li>User DN: <?php echo $users_dn ?></ul></td></tr>
	<tr><td colspan="2"><input type="submit" value="Next >>"></td></tr>
	</table>
	<?php 
		...
		echo '</div>';
	?>
	<input type="hidden" name="users_dn" value="<?php echo $users_dn ?>" /> 
...
function setDN($location, $dummydn) {
	...
	$udn = 'CN=[username],'.$bdn;
	return ' (<a href="#" onclick="setBaseDN(\''. $bdn .'\'); return false;">Base DN</a> ) ';
}
