...
function ajaxDoArticleSearch($query,$field='contentid') {
	...
	if(count($results)) {
		...
		$retval .= '<select name="created_by" class="inputbox">';
	        foreach($results as $result) {
			if(trim($result['title']) != '' && in_array($result['title'],$result)) {
				$uid = $result['id'];
				$retval .= "<option onClick=\"document.getElementById('preview').innerHTML='".
				str_replace("\r\n",'',str_replace('"','',str_replace("'",'',$result['introtext'])))
				."'; document.getElementById('".$field."').value='".$uid."'; lastsearchstring='".$result['title']."'\" value=\"".$uid."\">".$result['title']."</option>";
			}			
		}
		$retval .= '</select>';
	} else {
